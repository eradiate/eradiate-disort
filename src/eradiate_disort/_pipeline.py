...
- ``geometry``: dict with keys ``umu``, ``phi``, ``umu0``, ``phi0``, ``utau_altitudes``
...
from ._measurements import DisortRadianceMeasure
...
def aggregated_data(
    stacked_data: dict[str, xr.DataArray],
    mode: str,
    spectral_grid,
    ckd_quads,
...
def _build_radiance_dataset(
    ...
    """
    Build an xr.Dataset for a single DisortRadianceMeasure.
    ...
    measure_info : dict
        Per-measure metadata: ``utau_indices``, ``altitudes``, ``umu``, ``phi``.
    geometry : dict
        Solver geometry: ``umu0``, ``phi0``.

    Returns
    -------
    Dataset
        Variable ``uu`` with dims ``(w, z, vza, vaa)``.
        Coordinates: ``z`` [m], ``vza`` [deg], ``vaa`` [deg], ``sza`` [deg],
        ``saa`` [deg].
    """
    utau_idxs = measure_info["utau_indices"]
    altitudes: pint.Quantity = measure_info["altitudes"]
    umu: np.ndarray = measure_info["umu"]  # sorted cosines
    phi: np.ndarray = measure_info["phi"]  # azimuth in degrees
    umu0: float = geometry["umu0"]
    phi0: float = geometry["phi0"]

    # Slice radiance to this measure's utau levels
    # uu dims: (w[, g], umu_idx, utau_idx, phi_idx)
    uu_slice = uu.isel(utau_idx=utau_idxs)

    # Rename integer index dims to semantic names
    ...
    ds = uu_slice.to_dataset(name="uu")
    ds["uu"].attrs.update(
        {
...
def _build_irradiance_dataset(
    flux_fields: dict[str, xr.DataArray],
    measure_info: dict,
    geometry: dict,
) -> xr.Dataset:
    """
    Build an xr.Dataset for a single DisortIrradianceMeasure.
    ...
    for field, da in flux_fields.items():
        ...
        long_name, units = _FIELD_META.get(field, (field, ""))
...
def datatree(
    aggregated_data: dict[str, xr.DataArray],
    geometry: dict,
    measures_info: list[dict],
) -> xr.DataTree:
    """
    ...
    for minfo in measures_info:
        ...
        if minfo["type"] == "radiance" and uu is not None:
            subtrees[f"/{mid}"] = _build_radiance_dataset(uu, minfo, geometry)
        elif minfo["type"] == "irradiance":
            subtrees[f"/{mid}"] = _build_irradiance_dataset(
                flux_fields, minfo, geometry
            )
...
def build_disort_pipeline() -> Pipeline:
    ...
    - ``spectral_grid`` : :class:`~eradiate.spectral.grid.SpectralGrid`
    - ``ckd_quads`` : list of :class:`~eradiate.quad.Quad`
    - ``geometry`` : dict with ``umu``, ``phi``, ``umu0``, ``phi0``
    - ``measures_info`` : list of per-measure metadata dicts
...
def compute_measures_info(
    ...
    Parameters
    ----------
    measures : list of DisortRadianceMeasure or DisortIrradianceMeasure
    ...
    umu : ndarray
        Sorted unique polar-angle cosines for DISORT (from the radiance measure,
        if any).
    phi : ndarray
        Azimuth angles in degrees for DISORT (from the radiance measure, if any).
    ...
    measures_info : list of dict
        One entry per measure with keys ``id``, ``type``, ``utau_indices``,
        ``altitudes`` (and ``umu``, ``phi`` for radiance measures).
    """
    ...
    for m, utau_vals, alts in per_measure_utau:
        ...
        info: dict = {
            "id": m.id,
            "type": "radiance"
            if isinstance(m, DisortRadianceMeasure)
            else "irradiance",
            "utau_indices": idxs,
            "altitudes": alts,
        }
