...
class Pipeline:
    ...
    def write_png(
        ...
        """
        dot_graph = self._to_dot(highlight_nodes, legend=legend)
        dot_graph.write_png(filename)  # type: ignore[attr-defined]
    ...
    def write_svg(
        ...
        """
        dot_graph = self._to_dot(highlight_nodes, legend=legend)
        dot_graph.write_svg(filename)  # type: ignore[attr-defined]
    ...
    def print_summary(self) -> None:
        ...
        --------
        >>> pipeline = Pipeline()
        >>> pipeline.add_node("a", lambda: 1, description="Constant 1")
        >>> pipeline.add_node("b", lambda a: a + 1, dependencies=["a"],
        ...                   description="Add 1")
        >>> pipeline.print_summary()  # doctest: +SKIP
        Pipeline Summary
        ================
        Nodes: 2
        Validation: Enabled
