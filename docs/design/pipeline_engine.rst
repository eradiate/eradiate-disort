...
1. **Initial implementation** (commit ``d326641``): Core Pipeline/Node with
   ``bypass_data``, ``add_interceptor()``,
   ``pre_validators``/``post_validators``, separate visualization module.
2. **Virtual inputs** (commit ``0f2e421``): Added automatic virtual input
   detection, introspection methods, refactored to attrs.
3. **Visualization consolidation** (commits ``585377d``--``e22f3cf``):
   Integrated visualization into Pipeline, added legend support, Jupyter
   auto-display.
4. **API simplification** (commits ``5ef781a``--``2801956``): Renamed
   ``bypass_data`` to ``inputs``, removed ``add_interceptor()``, generalized
   validators to ``pre_funcs``/``post_funcs``.
