...
class TestDisortCkdMode:
    def test_molecular(self, benchmark, mode_ckd):
        benchmark(
            ed.EradiateDisortBackend().run, cases.molecular(sza=30.0, backend="disort")
        )

    def test_aerosols(self, benchmark, mode_ckd):
        benchmark(
            ed.EradiateDisortBackend().run, cases.aerosols(sza=30.0, backend="disort")
        )

    def test_full_atmo(self, benchmark, mode_ckd):
        benchmark(
            ed.EradiateDisortBackend().run, cases.full_atmo(sza=30.0, backend="disort")
        )
